...
# Команда вызова редактора (см. «Режим предпросмотра»)
edit_cmd: code

# Включение (или выключение) JS API
js_api: true

# Включение встроенного поиска
search: true

# Сохранять и показывать выдержки из статей
# в результатах поиска
keep_excerpts: true

# Индексировать контент на языках,
# обязательно массив, даже если язык один!
# Для одного языка будет: search_lang: ["ru"]
# Для поиска только на английском язык можно не указывать
search_lang: ["ru", "en"]
```
