...
Создает новую колонку или переписывает указанную, заменяя уникальные значения числовыми идентификаторами (id), начиная с единицы.
Порядок нумерации соответствует порядку появления значений в колонке.
...
- `input_col` — колонка, из содержимого которой будут сгенерированы id
- `output_col` — если указано, имя колонки, в которую будут помещены id. Если не указаны, будет переписана исходная колонка.


### hash **(0.1.4)**

Создает новую колонку на основе выбранной, в которую помещает хэш содержимого выбранной колонки. 
...
- `input_col` — колонка, на основе которой будет создана новая колонка
- `output_col` — имя новой колонки
- `long` — использовать более длинный хэш

### combine **(0.1.4)**

Создает новую колонку на основе нескольких выбранных. Содержимое выбранных колонок конкатенируется и хэшируется.  
...
- `input_cols` — список колонок, на основе которой будет создана новая колонка
- `output_col` — имя новой колонки
- `long` — использовать более длинный хэш
...
Примеры использования смотрите в разделе [«Примеры»](/+name:examples_intro_ru)
